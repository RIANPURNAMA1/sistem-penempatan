...
<div class="py-3">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-4">
        <div class="p-4 flex flex-wrap items-center justify-between gap-3">
            <h5 class="font-semibold text-gray-800"><i class="bi bi-building me-2"></i>Cabang</h5>
            <a href="{{ url('/cabang/create') }}" class="px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition text-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-3">
        <div class="overflow-x-auto">
            <table id="tableCabang" class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-gray-700 border-b border-gray-200">
                    <tr>
                        <th class="px-3 py-3 font-semibold text-center">No</th>
                        <th class="px-3 py-3 font-semibold">Nama cabang</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($cabangs as $index => $cabang)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-3 py-3 font-medium text-gray-800">{{ $cabang->nama_cabang }}</td>
                            <td class="px-3 py-3 text-gray-600">{{ $cabang->alamat }}</td>
                            <td class="px-3 py-3 text-gray-500">{{ $cabang->created_at->format('d/m/Y') }}</td>
                            <td class="px-3 py-3 text-center">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ url('/cabang/' . $cabang->id . '/edit') }}" class="p-1.5 bg-amber-100 text-amber-600 rounded hover:bg-amber-200 transition" title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <form action="{{ url('/cabang/' . $cabang->id) }}" method="POST" class="inline delete-form">
                                        @csrf @method('DELETE')
